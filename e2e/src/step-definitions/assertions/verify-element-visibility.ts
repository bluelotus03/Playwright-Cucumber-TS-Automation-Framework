...
Then(
    ...
    async function(this: ScenarioWorld, elementPosition: string, elementKey: string, negate: boolean) {
        const {
            screen: { page },
            globalConfig,
        } = this
        ...
        await waitFor( async () => {
            ...
            globalConfig,
            { target: elementKey, failureMessage: `❗️ Expected ${elementPosition} ${elementKey} to ${negate?'not ':''}be displayed` }
        )
    }
)
...
Then(
    ...
    async function(this: ScenarioWorld, negate: boolean, count: string, elementKey: ElementKey) {
        ...
        await waitFor( async () => {
            ...
            globalConfig,
            { target: elementKey, failureMessage: `❗️ Expected ${count} ${elementKey} to ${negate?'not ':''}be displayed` }
        )
    }
)